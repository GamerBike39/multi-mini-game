...
const PROFILES: Record<string, GameMusicProfile> = {
  ...
  breaker: profile('shooter', {
    intensity: 0.42, tension: 0.28, danger: 0.12, momentum: 0.5, complexity: 0.35,
    brightness: 0.55, triumph: 0, calm: 0.3, narrativeArc: 0.1,
  }),
  run: profile('runner', {
  ...
  golf: profile('cave', {
    intensity: 0.22, tension: 0.2, danger: 0.05, momentum: 0.26, complexity: 0.28,
    brightness: 0.5, triumph: 0, calm: 0.58, narrativeArc: 0.12,
  }),
  snake: profile('runner', {
...
export class GameMusicAdapter {
  ...
  private metrics(source: GameMusicSource): MusicState {
    ...
    } else if (this.profile.mood === 'runner') {
    ...
    } else if (this.profile.mood === 'simon') {
...
export class MenuMusicAdapter {
  ...
  update(_dt: number): void {
    if (!this.active) return;
    this.audio.setMusicState({ calm: 0.78, intensity: 0.16, danger: 0.02 });
  }
